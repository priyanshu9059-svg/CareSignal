export async function api<T = any>(path:string, options:RequestInit={}):Promise<T> {
  ...
  const response=await fetch('/api'+path,{...options,signal:options.signal||timeout,credentials:'same-origin',headers:{...(options.body instanceof FormData?{}:{'Content-Type':'application/json'}),...options.headers}})
    .catch(error=>{throw new Error(error?.name==='TimeoutError'?'The service did not respond. Check that both local servers are running.':'Unable to connect to the service.');});
...
export type User={id:string;name:string;email:string;role:string;district_id:string;state_id:string;consent:any;demo_enabled:boolean};
