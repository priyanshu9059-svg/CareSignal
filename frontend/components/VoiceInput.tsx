...
export default function VoiceInput({caseId,language,t,onReady,onTranscript}:{caseId:string;language:string;t:any;onReady:(id:string)=>void;onTranscript:(text:string)=>void}) {
 ...
 async function start(){try{
  ...
  if(SR){const r=new SR();recognition.current=r;r.lang=language==='en'?'en-IN':'hi-IN';r.continuous=true;r.onresult=(e:any)=>{transcript.current=Array.from(e.results).map((v:any)=>v[0].transcript).join(' ');onTranscript(transcript.current);};r.onerror=()=>setMessage('Browser transcription unavailable. Please type the transcript.');r.start();}
  else setMessage('Browser transcription unavailable. Please type the transcript.');
 ...
 async function stop(){
  ...
  const form=new FormData();form.append('file',new Blob([buffer],{type:'audio/wav'}),'checkin.wav');form.append('case_id',caseId);form.append('transcript',transcript.current);
  try {setMessage('Processing recording…');const result=await api('/ai/analyze-voice',{method:'POST',body:form});onReady(result.voice_session_id);setMessage(`Recording processed (${result.duration}s). ${result.baseline_note}`);}
