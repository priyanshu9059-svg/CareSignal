...
def seed(db):
    if db.get(User,'U-victim'): return
    rng=random.Random(26094)
    for role in ['victim','counsellor','officer','state','national','admin']:
        db.add(Role(id=role,name=role))
    for id,name in [('S1','Madhya Pradesh'),('S2','Rajasthan'),('S3','Maharashtra')]: db.add(State(id=id,name=name))
    db.flush()
    ...
    for id,name,state,lat,lon in districts: db.add(District(id=id,name=name,state_id=state,latitude=lat,longitude=lon))
    db.flush()
    import os
    ...
    db.flush()
    for i in range(50): db.add(Victim(id=f'V-{i}',user_id='U-victim' if i==0 else f'U-victim-{i}',alias=f'Participant {i+1:03}'))
    db.flush()
    for i in range(50):
        d=districts[i%6][0]
    ...
    for i in range(50):
        ...
        for week in range(12):
            if i==0: score=[27,29,30,31,32,39,47,58,74,78,63,49][week]
            else:
                pattern=i%9
