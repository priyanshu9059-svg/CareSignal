...
def dashboard(level:str,user:User=Depends(current_user),db:Session=Depends(get_db)):
    ...
    if level not in allowed or user.role not in allowed[level]: raise HTTPException(403,'Dashboard outside your role')
    query=select(Case).join(District)
    if level=='district' and user.role!='admin': query=query.where(Case.district_id==user.district_id)
    if level=='district' and user.role=='counsellor': query=query.where(Case.assigned_to==user.id)
    if level=='state' and user.role!='admin': query=query.where(District.state_id==user.state_id)
    cases=list(db.scalars(query)); ids=[c.id for c in cases]
    ...
    for c in summaries: grouped[c['district_id']].append(c)
    districts=[]
    ...
    audit(db,user,'view aggregate dashboard',level); db.commit()
...
def scenario(name:str,user:User=Depends(roles('victim')),db:Session=Depends(get_db)):
    if not DEMO_ENABLED: raise HTTPException(404,'Demo mode disabled')
    ...
    if name not in choices: raise HTTPException(404,'Unknown scenario')
